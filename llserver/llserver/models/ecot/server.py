...
import asyncio
from ECoTModel import ECoTModel  # Импорт вашего класса LlavaModel
...
logger = CustomLogger(mode="logging", log_file="/llserver/logs/ecot.log")  # Можно переключить на "wandb"
...
async def lifespan(app: FastAPI):
    ...
    model = ECoTModel(logger=logger)  # Передаем логгер в класс модели
...
async def put_task(
    ...
    model: ECoTModel = Depends(get_model),
):
    """
