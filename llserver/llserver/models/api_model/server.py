...
import asyncio
from APIModel import APIModel  # Импорт вашего класса LlavaModel
...
logger = CustomLogger(mode="logging", log_file="/llserver/logs/api_model.log")  # Можно переключить на "wandb"
...
async def lifespan(app: FastAPI):
    ...
    model = APIModel(logger=logger)  # Передаем логгер в класс модели
...
async def put_task(
    ...
    model: APIModel = Depends(get_model),
):
    """
...
@app.post("/get_task_result_async/")
