...
import asyncio
from CogActModel import CogActModel  # Импорт вашего класса CogActModel
...
logger = CustomLogger(mode="logging", log_file="/llserver/logs/cogact.log")  # Можно переключить на "wandb"
...
async def lifespan(app: FastAPI):
    ...
    model = CogActModel(logger=logger)  # Передаем логгер в класс модели
...
async def put_task(
    ...
    model: CogActModel = Depends(get_model),
):
    """
